...
describe('datecalc.ageInWeeks(fromDt, toDt)', function(){
  it('should throw error if args not Date objects', function(){
    (function() {datecalc.ageInWeeks(25, new Date(2012,7,11))}).should.throw();
    (function() {datecalc.ageInWeeks(new Date(2010,0,01), "25/6/2012")}).should.throw();
  })
})
describe('datecalc.ageInWeeks(fromDt, toDt)', function(){
  it('should throw error if toDt not after fromDt', function(){
    (function() {datecalc.ageInWeeks(new Date(2012,8,1), new Date(2012,7,11))}).should.throw();
  })
})

describe('datecalc.ageInWeeks(fromDt, toDt)', function(){
  it('should calculate age in weeks between 2 dates', function(){
    datecalc.ageInWeeks(new Date(2012,7,01), new Date(2012,7,6))
    	.should.include({weeks: 0, days: 5});
    datecalc.ageInWeeks(new Date(2012,7,01), new Date(2012,7,8))
    	.should.include({weeks: 1, days: 0});
    datecalc.ageInWeeks(new Date(2010,5,01), new Date(2010,7,01))
      .should.include({weeks: 8, days: 5});
  })
})
